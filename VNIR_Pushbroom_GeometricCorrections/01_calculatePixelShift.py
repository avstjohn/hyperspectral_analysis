...
for i in range(nLineScans):
	...
	nPixelPerShift.append(int(m.ceil(diff_lon_meters[i]/ pixelWidth_meter[i])))

maxShift = max(nPixelPerShift)
minShift = min(nPixelPerShift)
...
lat_buffer = (0.1*nPixelsPerLineScan + maxShift) * pixelLength_deg[0]
...
#  Populate (latitude, longitude) "lookup" array 
lookup = np.zeros((box.shape[0], box.shape[1], 2))

lookup[0, 0, 0] = boxV4[0]
lookup[0, 0, 1] = boxV4[1]

lookup[-1, -1, 0] = boxV2[0]
lookup[-1, -1, 1] = boxV2[1]
'''
for i in range(nPixel1):
	for j in range(nPixel2):
		lat = 
...
#  If box[i, j] is a trajectory pixel, then flip it to '1'


'''
#  Import BIL to embed into 'box' array
...
arr = img.load()[:, :, 0]
print(arr.shape)
nLineScans = arr.shape[0]
nPixelsPerLineScan = arr.shape[1]
'''
...
with open('pixelShift.csv', 'w') as output:
	...
	writer.writerows(izip(nPixelPerShift))
